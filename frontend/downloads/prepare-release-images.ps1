...
$portalImage = if ($env:SAFEAGENT_FRONTEND_IMAGE) { $env:SAFEAGENT_FRONTEND_IMAGE } else { "ghcr.io/our-org/safeagent-dashboard:v0.2.0" }
$apiTar = "safeagent-api-v0.2.0.tar"
$portalTar = "safeagent-dashboard-v0.2.0.tar"

Write-Host "[SafeAgent] API 이미지를 가져옵니다: $apiImage"
docker pull $apiImage
docker save -o $apiTar $apiImage

Write-Host "[SafeAgent] 대시보드 이미지를 가져옵니다: $portalImage"
