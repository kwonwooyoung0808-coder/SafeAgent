...
DASHBOARD_IMAGE="${SAFEAGENT_FRONTEND_IMAGE:-ghcr.io/our-org/safeagent-dashboard:v0.2.0}"

echo "[SafeAgent] API 이미지를 가져옵니다: ${API_IMAGE}"
docker pull "${API_IMAGE}"
docker save -o safeagent-api-v0.2.0.tar "${API_IMAGE}"

echo "[SafeAgent] 대시보드 이미지를 가져옵니다: ${DASHBOARD_IMAGE}"
docker pull "${DASHBOARD_IMAGE}"
docker save -o safeagent-dashboard-v0.2.0.tar "${DASHBOARD_IMAGE}"
