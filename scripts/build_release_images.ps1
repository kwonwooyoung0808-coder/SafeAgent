...
$portalImage = "safeagent-dashboard-local:$version"
$downloadsDir = Join-Path $PSScriptRoot "..\frontend\downloads"
$apiTar = Join-Path $downloadsDir "safeagent-api-$version.tar"
$portalTar = Join-Path $downloadsDir "safeagent-dashboard-$version.tar"
...
Write-Host "[SafeAgent] 대시보드 이미지를 로컬에서 빌드합니다: $portalImage"
...
Write-Host "[SafeAgent] 대시보드 이미지를 tar로 저장합니다: $portalTar"
